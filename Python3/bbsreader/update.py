...
### 读取新数据
def update_threads(crawler, boardId, threads, start, pages):
...
def download_article(crawler, t):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "start", nargs="?", type=int, help="manual set download start page"
    )
    parser.add_argument("-l", "--list", action="store_true", help="support BBS list")
    parser.add_argument("--bbsid", type=int, default=None, help="manual set <BBS ID>")
    parser.add_argument(
        "--boardid", type=int, default=None, help="manual set <BOARD ID>"
    )
    ...
    if args.list:
        ...
        main(pages, start, bbsId, boardId)
