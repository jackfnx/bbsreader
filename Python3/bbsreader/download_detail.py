...
parser.add_argument(
    "-t", "--title", nargs="?", type=str, help="manual set title (when update index)"
)
parser.add_argument(
    "-a", "--author", nargs="?", type=str, help="manual set author (when update index)"
)
parser.add_argument(
    "-p",
    "--posttime",
    nargs="?",
    type=str,
    help="manual set post time (when update index)",
)
...
for threadId in threadIds:
    text, title, author, postTime, postUrl, page_num, floors = getpage(
        crawler, threadId, 1
    )
    ...
    for page in range(2, page_num2 + 1):
        ...
        print(
            "thread: %s, page: %d/%d, new floors: %s."
            % (threadId, page, page_num, floors)
        )
        text += "\n\n\n\n-------------------------------------------------\n\n\n\n"
        text += nextPage
    ...
    new_threads.append(
        MakeThread(crawler.siteId, threadId, title, author, postTime, postUrl)
    )
    print(new_threads[-1])
...
if updateIndex:
    ...
    if asSingles:
        sk = [
            x
            for x in meta_data.superkeywords
            if x["skType"] == SK_Type.Manual and x["keyword"] == "*"
        ][0]
        ...
        if tid not in sk["tids"]:
            ...
            ret = "existed"
        sk["tids"].sort(
            key=lambda x: meta_data.last_threads[x]["postTime"], reverse=True
        )
