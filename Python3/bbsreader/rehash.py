...
def main():
    ...
    parser.add_argument(
        "threadids", nargs="+", type=int, default=[], help="<Thread ID>"
    )
    parser.add_argument("--url", nargs="?", type=str, help="<URL>")
    parser.add_argument(
        "--sk", nargs="?", type=int, default=-1, help="<Superkeyword ID>"
    )
    parser.add_argument(
        "--printonly", "-P", action="store_true", help="print superkeyword id only."
    )
    parser.add_argument(
        "--title", nargs="?", type=str, help="manual set title (when update index)"
    )
    parser.add_argument(
        "--author", nargs="?", type=str, help="manual set author (when update index)"
    )
    ...
    if 0 <= args.sk < len(meta_data.superkeywords):
        ...
        ts = [
            (args.bbsid, str(t), "thread-%s-%d-1.html" % (t, 1)) for t in args.threadids
        ]
