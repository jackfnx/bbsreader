...
def main():
    ...
    # 临时存储tids
    restore_threads = {
        i: [meta_data.last_threads[y] for y in x["tids"] if y not in objects]
        for (i, x) in enumerate(meta_data.superkeywords)
    }

    meta_data.last_threads = [
        x for (i, x) in enumerate(meta_data.last_threads) if i not in objects
    ]
