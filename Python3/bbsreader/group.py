    ...
    for i in range(len(keys)):
        ...
        for j in range(i + 1, len(ids)):
            ...
            if text1 != "" and text2 != "":
                ### 如果长度差的太多，不用比较内容
                lenr = (
                    len(text1) * 1.0 / len(text2)
                    if len(text1) >= len(text2)
                    else len(text2) * 1.0 / len(text1)
                )
                if lenr > 1.5:
                    continue
                ### 比较标题相似度
                ...
                if tr < 90 and not (
                    pure_title1 in pure_title2 or pure_title2 in pure_title1
                ):
                    continue
                ## 比较内容相似度
...
def do_grouping(threads, superkeyword, save_root_path, silence=True):
    ...
    t_01 = time.time()
    print(
        "superkeyword [%s]: grouped. (%d match, %d new, %.2fs)"
        % (keytext(superkeyword), len(superkeyword["groups"]), inc, t_01 - t_00)
    )
    if not silence:
        print("The result:")
        print_groups(superkeyword["groups"])
...
def main():
    ...
    parser.add_argument(
        "-k", "--kid", type=int, default=-1, help="manual set superkeyword"
    )
    parser.add_argument(
        "-p", "--printonly", action="store_true", help="only print, not save"
    )
    ...
    if args.list:
        print(
            ...
            % "\n".join(
                [
                    "%-3d: %s" % (x, keytext(superkeywords[x]))
                    for x in range(len(superkeywords))
                ]
            )
        )
        sys.exit(0)
